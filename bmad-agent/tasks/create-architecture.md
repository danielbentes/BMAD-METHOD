...
- To produce all necessary technical artifacts at `.bmad/current/specs/architecture.md`, ensuring the architecture is optimized for efficient implementation, particularly by AI developer agents, and validated against the `architect-checklist`.
...
6.  **Validate Architecture Against Checklist & Finalize Output:**
    ...
    - **Recommend Next Steps for UI (If Applicable):**
      - After the main architecture document is finalized and approved at `.bmad/current/specs/architecture.md`:
      - If the project involves a user interface (as should be evident from the input PRD and potentially the architecture document itself mentioning UI components or referencing outputs from a Design Architect's UI/UX Specification phase):
        ...
        - Explain that the Design Architect will use the now-completed main Architecture Document at `.bmad/current/specs/architecture.md` and the detailed UI/UX specifications (e.g., `.bmad/current/specs/frontend-spec.md` or enriched PRD) as primary inputs to define the specific frontend architecture, select frontend libraries/frameworks (if not already decided), structure frontend components, and detail interaction patterns.
...
- Optionally, if UI components are involved and the user agrees: A prompt for a "Design Architect" appended to the main architecture document at `.bmad/current/specs/architecture.md`, summarizing relevant UI considerations and outlining the Design Architect's next steps.
