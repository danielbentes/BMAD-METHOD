...
Transform large project documents into smaller, granular files within the `.bmad/current/specs/` directory following the `doc-sharding-tmpl.txt` plan. Create and maintain `.bmad/current/specs/index.md` as a central catalog for easier reference and context injection.
...
2. Validate prerequisites:
   ...
   - Write access to the `.bmad/current/specs/` directory
...
4. For `.bmad/current/specs/index.md` when working with the file system:
...
5. Store sharding results in `.bmad/current/specs/` directory structure
...
4. **Output Location** - Confirm Target directory (default: `.bmad/current/specs/`) and index.md or in memory chat output
